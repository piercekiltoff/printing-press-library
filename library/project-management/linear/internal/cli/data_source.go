...
func resolveLocal(resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	...
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'linear-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'linear-pp-cli sync' first", resourceType)
		}
