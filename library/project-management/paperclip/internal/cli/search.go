...
In local mode: searches locally synced data only.`,
  ...
  paperclip-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  paperclip-pp-cli search "critical" --type transactions --data-source local
  ...
  paperclip-pp-cli search "critical" --json --limit 20`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch resourceType {
			case "environments":
				results, err = db.SearchEnvironments(query, limit)
			case "companies":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchEnvironments(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search environments failed: %w", searchErr)
					}
