		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export PAPERCLIP_BOARD_SESSION_AUTH=<your-key>"
				} else {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if authHeader == "" {
					...
					authReq.Header.Set("paperclip_session", authHeader)
					authReq.Header.Set("User-Agent", "paperclip-pp-cli")
...
func collectCacheReport(staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'paperclip-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
