...
In local mode: searches locally synced data only.`,
  ...
  scrape-creators-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  scrape-creators-pp-cli search "critical" --type transactions --data-source local
  ...
  scrape-creators-pp-cli search "critical" --json --limit 20`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, getErr := c.Get("/v1/google/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
	...
	cmd.AddCommand(newSearchTrendsCmd(flags))
