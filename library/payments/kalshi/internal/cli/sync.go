...
func extractPageItems(data json.RawMessage, cursorParam string) ([]json.RawMessage, string, bool) {
	...
	// Try common item keys, including Kalshi-specific response wrapper keys
	itemKeys := []string{"data", "results", "items", "records", "nodes", "entries",
		"markets", "events", "series", "fills", "settlements", "orders",
		"positions", "event_positions", "market_positions", "trades",
		"milestones", "announcements", "api_keys", "rfqs", "quotes",
		"structured_targets", "incentive_programs", "fee_changes",
		"collections", "order_groups", "subaccount_balances", "transfers"}
...
func syncResourcePath(resource string) string {
	paths := map[string]string{
		...
		"search":                         "/search/filters_by_sport",
		"series":                         "/series",
...
func extractID(obj map[string]any) string {
	for _, key := range []string{"id", "ID", "ticker", "event_ticker", "series_ticker", "uuid", "slug", "name"} {
		if v, ok := obj[key]; ok {
			s := fmt.Sprintf("%v", v)
			if s != "" && s != "<nil>" {
				return s
