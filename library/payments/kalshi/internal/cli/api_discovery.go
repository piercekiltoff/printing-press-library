...
Run 'api <interface>' to see that interface's methods.`,
  ...
  kalshi-pp-cli api <interface-name>`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if flags.dryRun {
				fmt.Fprintln(cmd.OutOrStdout(), "GET /api (list interfaces)")
				return nil
			}
			root := cmd.Root()
			...
			if len(args) > 0 {
				...
				for _, child := range root.Commands() {
					if child.Hidden && strings.ToLower(child.Name()) == target {
						...
						fmt.Fprintf(cmd.OutOrStdout(), "\nUse '%s-pp-cli %s <method> --help' for details.\n", "kalshi-trade-manual", child.Name())
						return nil
					}
				}
				return fmt.Errorf("interface %q not found. Run '%s-pp-cli api' to list all interfaces", args[0], "kalshi-trade-manual")
			}
