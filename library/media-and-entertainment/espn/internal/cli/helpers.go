...
func classifyAPIError(err error) error {
	...
	case strings.Contains(msg, "HTTP 401") || strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials. Run 'espn-pp-cli doctor' to verify auth, or set the required environment variable", err))
...
func confirmMutation(flags *rootFlags, stderr io.Writer, prompt string) error {
	...
	if flags.noInput {
		return usageErr(fmt.Errorf("%s: rerun with --yes to confirm in non-interactive mode", prompt))
	}
	...
	if answer != "y" && answer != "yes" {
		return usageErr(fmt.Errorf("operation canceled"))
	}
	return nil
}
