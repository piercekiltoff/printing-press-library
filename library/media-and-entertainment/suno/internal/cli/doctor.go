...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	var failOn string
	// PATCH: optional zero-credit generate probe distinguishes API reachability from generate-path interstitials.
	var probeGenerate bool
	cmd := &cobra.Command{
		Use:   "doctor",
  ...
  suno-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "suno-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = "suno.com"
			...
			if os.Getenv("SUNO_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SUNO_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SUNO_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if probeGenerate {
						report["generate_path_probe"] = probeGeneratePath(c)
					}
				}
			...
			checkKeys := []struct{ key, label string }{
				...
				{"generate_path_probe", "generate path probe"},
			}
	...
	cmd.Flags().BoolVar(&probeGenerate, "probe-generate", false, "Probe zero-credit lyrics generation path")
	return cmd
}

// PATCH: classify the reverse-engineered generation path without spending credits.
func probeGeneratePath(c interface {
	Post(string, any) (json.RawMessage, int, error)
}) string {
	data, _, err := c.Post("/api/generate/lyrics/", map[string]any{"prompt": "test"})
	if err == nil {
		if looksLikeDoctorInterstitial(data) != "" || !json.Valid(data) {
			return "captcha_intercepted"
		}
		return "reachable"
	}
	...
	if errors.As(err, &apiErr) {
		...
		if looksLikeDoctorInterstitial([]byte(apiErr.Body)) != "" || strings.Contains(strings.ToLower(apiErr.Body), "<html") {
			return "captcha_intercepted"
		}
...
func browserSessionProofStatus(cfg *config.Config, authHeader string) (bool, string) {
	...
	if proof.APIName != "suno" {
		return false, "proof belongs to a different CLI"
	}
	if proof.CookieDomain != "suno.com" {
		return false, "proof belongs to a different cookie domain"
	}
	if proof.ValidationPath != "/api/billing/eligible-discounts" {
