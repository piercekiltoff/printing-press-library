...
Use --chrome to read cookies from Chrome for suno.com.
...
profile by name when the installed backend supports it.`,
  ...
  suno-pp-cli auth login --browser
  suno-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  suno-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  suno-pp-cli auth login --browser\n")
				return nil
			}
			...
			requiredCookies := []string{"__session"}
...
func newAuthRefreshCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  suno-pp-cli auth refresh",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func browserSessionTargetURL(cfg *config.Config) string {
	baseURL := strings.TrimRight("https://studio-api-prod.suno.com", "/")
	if cfg != nil && cfg.BaseURL != "" {
		baseURL = strings.TrimRight(cfg.BaseURL, "/")
	}
	validationPath := strings.TrimSpace("/api/billing/eligible-discounts")
	if validationPath == "" {
		return baseURL
	}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  suno-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: suno.com\n")
...
// PATCH(greptile #577 round 5): drop the stale exec.LookPath("sqlite3") guard.
// Round 4 replaced the shell-out in countCookiesForDomain with the modernc/sqlite
// driver via sql.Open, so the sqlite3 CLI binary is no longer a dependency.
// Leaving the guard in place caused Linux systems without sqlite3 installed
// (Docker, CI, bare Ubuntu) to fall back to profile-less extraction, which
// silently disabled Chrome profile auto-detection -- the exact problem the
// round-4 fix was meant to solve.
...
// Uses the in-process modernc/sqlite driver (no sqlite3 CLI dependency); host_key is plaintext so no decryption is needed.
...
func validateComposedAuth(authHeader string) error {
	testURL := "https://studio-api-prod.suno.com"
...
func browserSessionProofPath(cfg *config.Config) string {
	if cfg == nil || cfg.Path == "" {
		...
		return filepath.Join(home, ".config", "suno-pp-cli", "browser-session-proof.json")
	}
...
func browserSessionProofStatusForAuth(cfg *config.Config, authHeader string) (bool, string) {
	...
	if err != nil {
		return false, "proof not found; run suno-pp-cli auth login --chrome"
	}
	var proof browserSessionProof
	if err := json.Unmarshal(data, &proof); err != nil {
		return false, "proof is not valid JSON; re-run suno-pp-cli auth login --chrome"
	}
	if proof.APIName != "suno" || proof.CookieDomain != "suno.com" || proof.ValidationPath != "/api/billing/eligible-discounts" {
