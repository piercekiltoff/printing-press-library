...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Sync trending and popular movies/TV into a local SQLite database for offline
search. Fetches up to --max-pages pages per resource (default 5, ~100 items each).
Your local DB also grows automatically as you use the CLI — every live API lookup
is cached for FTS search.`,
		Example: `  # Sync trending and popular (default: 5 pages per resource)
  movie-goat-pp-cli sync

  # Sync more data
  movie-goat-pp-cli sync --max-pages 20

  # Full resync (ignore previous checkpoint)
  movie-goat-pp-cli sync --full

  # Sync specific resources only
  movie-goat-pp-cli sync --resources trending,movies`,
	...
	cmd.Flags().IntVar(&maxPages, "max-pages", 5, "Maximum pages to fetch per resource (0 = unlimited)")
...
func syncResource(c interface {
	...
	for {
		...
		// Enforce page ceiling to prevent runaway syncs on large catalogs
