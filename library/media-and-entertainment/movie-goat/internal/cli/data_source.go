...
// FTS search covers everything you've ever looked up — not just explicit syncs.
...
func writeThroughCache(resourceType string, data json.RawMessage) {
	db, err := store.Open(defaultDBPath("movie-goat-pp-cli"))
	if err != nil {
		return
	}
	defer db.Close()
	...
	if json.Unmarshal(data, &items) != nil || len(items) == 0 {
		items = nil
		// Try object — check for TMDb-style envelope with "results" array
		...
		if json.Unmarshal(data, &envelope) == nil {
			...
			// Single object with an id field (e.g., movie detail response)
			if items == nil {
...
func resolveLocal(resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead("movie-goat-pp-cli")
	if err != nil {
