...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "servicetitan-memberships-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "servicetitan-memberships-pp-cli/1.0")
		}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	// PATCH: composed-auth-apikey-dryrun: include the ST-App-Key header in
	// --dry-run output so users can verify both halves of composed auth.
	if c.Config != nil && c.Config.StAppKey != "" {
...
func (c *Client) mintClientCredentials(clientID, clientSecret string) error {
	...
	// PATCH: client-drop-dead-tokenurl-guard (Greptile PR #601 P2): the
	// preceding block unconditionally fills tokenURL, so the previous
	// `if tokenURL == ""` early-return here was unreachable. Removed.
	form := url.Values{
