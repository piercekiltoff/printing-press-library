...
Exit codes & warnings:
  ...
  servicetitan-memberships-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  servicetitan-memberships-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  servicetitan-memberships-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  servicetitan-memberships-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  servicetitan-memberships-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "duration_billing_items":
		return db.UpsertDurationBillingItems(data)
	case "recurring_service_items":
		return db.UpsertRecurringServiceItems(data)
	case "status_changes":
		return db.UpsertStatusChanges(data)
	case "mark_complete":
		return db.UpsertMarkComplete(data)
	case "mark_incomplete":
		return db.UpsertMarkIncomplete(data)
	default:
...
// PATCH: sync-registry (#1305): generator emitted empty registry stubs; the
// six cacheable Memberships v2 resources are wired here so `sync` and the
// per-resource list/cache commands and the novel audit commands all read/
// write the same store keys.
...
// Every Memberships v2 path is tenant-scoped; ST_TENANT_ID is substituted
// in for the `{tenant}` placeholder so sync can run without requiring the
// caller to pass a tenant positional arg every time.
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"memberships":              "/tenant/{tenant}/memberships",
		"membership-types":         "/tenant/{tenant}/membership-types",
		"recurring-services":       "/tenant/{tenant}/recurring-services",
		"recurring-service-types":  "/tenant/{tenant}/recurring-service-types",
		"recurring-service-events": "/tenant/{tenant}/recurring-service-events",
		"invoice-templates":        "/tenant/{tenant}/invoice-templates",
	}
	p, ok := paths[resource]
	if !ok {
