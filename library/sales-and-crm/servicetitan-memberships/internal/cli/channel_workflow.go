...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  servicetitan-memberships-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  servicetitan-memberships-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  servicetitan-memberships-pp-cli workflow status

  # Show status as JSON
  servicetitan-memberships-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("servicetitan-memberships-pp-cli")
			}
