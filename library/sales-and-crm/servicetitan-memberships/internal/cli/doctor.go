...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servicetitan-memberships-pp-cli doctor
  servicetitan-memberships-pp-cli doctor --json
  servicetitan-memberships-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Check auth.
			// PATCH: composed-auth-doctor (#1303): the bearer is minted by
			// the auth-login flow on first API call, so cfg.AuthHeader() is
			// empty at doctor time even when auth is fully configured.
			// Recognize composed auth from the credentials actually present
			// (ST-App-Key + client_id/secret), not from a not-yet-minted bearer.
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'servicetitan-memberships-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'servicetitan-memberships-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'servicetitan-memberships-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'servicetitan-memberships-pp-cli sync' to refresh."
	}
	return report
}
