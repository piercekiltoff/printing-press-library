...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".servicetitan-memberships-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SERVICETITAN_MEMBERSHIPS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SERVICETITAN_MEMBERSHIPS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "servicetitan-memberships-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.servicetitan-memberships-pp-cli/feedback.jsonl.
When ` + "`SERVICETITAN_MEMBERSHIPS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SERVICETITAN_MEMBERSHIPS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "servicetitan-memberships-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servicetitan-memberships-pp-cli feedback list
  servicetitan-memberships-pp-cli feedback list --limit 5
  servicetitan-memberships-pp-cli feedback list --json`,
