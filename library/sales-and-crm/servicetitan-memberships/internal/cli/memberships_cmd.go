...
// Shared scaffolding for the transcendence commands (renewals, expiring,
// overdue-events, schedule, drift, risk, revenue, health, stale-services,
// bill-preview, complete, find). These commands are hand-written on top of
// the internal/memberships foundation package; the generated endpoint
// commands are unaffected.

// openMembershipsStore opens the local SQLite store for a transcendence
...
func openMembershipsStore(cmd *cobra.Command, dbPath string) (*store.Store, error) {
	if dbPath == "" {
		dbPath = defaultDBPath("servicetitan-memberships-pp-cli")
	}
	db, err := store.OpenWithContext(cmd.Context(), dbPath)
	if err != nil {
		return nil, fmt.Errorf("opening local database: %w\nRun 'servicetitan-memberships-pp-cli sync' first to populate it", err)
	}
	empty, err := memberships.StoreEmpty(db)
	if err != nil {
		db.Close()
		return nil, err
	}
	if empty {
		db.Close()
		return nil, fmt.Errorf("local store is empty — run 'servicetitan-memberships-pp-cli sync' first")
	}
	return db, nil
}

// mbOutput renders a transcendence command's result: a JSON envelope (routed
// through --select / --compact / --csv / --quiet) when --json is set or
// stdout is not a terminal, otherwise a human-readable table.
func mbOutput(cmd *cobra.Command, flags *rootFlags, jsonVal any, headers []string, rows [][]string) error {
