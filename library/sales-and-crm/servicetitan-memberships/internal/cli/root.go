...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "servicetitan-memberships-pp-cli",
		Short: `Servicetitan Memberships CLI — A focused per-module ServiceTitan CLI for Memberships — sync once, then audit renewals, overdue events, template drift,…`,
		Long: `Servicetitan Memberships CLI — A focused per-module ServiceTitan CLI for Memberships — sync once, then audit renewals, overdue events, template drift,…
...
Health check: run 'servicetitan-memberships-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("servicetitan-memberships-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'servicetitan-memberships-pp-cli profile list')")
	...
	rootCmd.AddCommand(newInvoiceTemplatesCmd(flags))
	rootCmd.AddCommand(newMembershipTypesCmd(flags))
	rootCmd.AddCommand(newMembershipsCmd(flags))
	rootCmd.AddCommand(newMembershipsExportCmd(flags))
	rootCmd.AddCommand(newRecurringServiceTypesCmd(flags))
	rootCmd.AddCommand(newRecurringServicesCmd(flags))
	...
	rootCmd.AddCommand(newRecurringServiceEventsPromotedCmd(flags))
	rootCmd.AddCommand(newHealthCmd(flags))
	rootCmd.AddCommand(newRenewalsCmd(flags))
	rootCmd.AddCommand(newExpiringCmd(flags))
	rootCmd.AddCommand(newOverdueEventsCmd(flags))
	rootCmd.AddCommand(newScheduleCmd(flags))
	rootCmd.AddCommand(newDriftCmd(flags))
	rootCmd.AddCommand(newRiskCmd(flags))
	rootCmd.AddCommand(newRevenueCmd(flags))
	rootCmd.AddCommand(newStaleServicesCmd(flags))
	rootCmd.AddCommand(newBillPreviewCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("servicetitan-memberships-pp-cli %s\n", version)
		},
	}
}
