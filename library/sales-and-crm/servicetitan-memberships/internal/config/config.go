...
type Config struct {
	...
	// TenantID is the numeric ServiceTitan tenant ID. Every Memberships path
	// is tenant-scoped (/tenant/{tenant}/...). PATCH tenant-env-config (#1332).
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.servicetitan.io/memberships/v2",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("SERVICETITAN_MEMBERSHIPS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "servicetitan-memberships-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// PATCH: tenant-env-config (#1332): every Memberships path is tenant-scoped.
	...
	if v := os.Getenv("SERVICETITAN_MEMBERSHIPS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("SERVICETITAN_MEMBERSHIPS_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
