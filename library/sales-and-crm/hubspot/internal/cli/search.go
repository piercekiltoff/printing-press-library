...
In local mode: searches locally synced data only.`,
  ...
  hubspot-pp-cli search "error timeout"

  # Force local search only
  hubspot-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  hubspot-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  hubspot-pp-cli search "critical" --json --limit 20`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
			query := args[0]
			// This API has a search endpoint: POST /crm/v3/objects/deals/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/crm/v3/objects/deals/search", map[string]any{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'hubspot-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
