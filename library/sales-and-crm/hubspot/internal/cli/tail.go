...
native streaming instead of polling.`,
  ...
  hubspot-pp-cli tail --interval 10s

  # Tail deal changes every 5 seconds
  hubspot-pp-cli tail deals --interval 5s

  # Pipe to jq for filtering closed-won deals
  hubspot-pp-cli tail deals --interval 30s | jq 'select(.data.properties.dealstage == "closedwon")'`,
