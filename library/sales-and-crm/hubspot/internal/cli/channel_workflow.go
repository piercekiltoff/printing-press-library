...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  hubspot-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  hubspot-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"calls", "companies", "contacts", "deals", "emails", "lists", "meetings", "notes", "owners", "pipelines", "properties", "tasks", "tickets"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  hubspot-pp-cli workflow status

  # Show status as JSON
  hubspot-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("hubspot-pp-cli")
			}
