# ServiceTitan CRM Printed CLI Agent Guide

This directory is a generated `servicetitan-crm-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
servicetitan-crm-pp-cli doctor --json
servicetitan-crm-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
servicetitan-crm-pp-cli which "<capability>" --json
servicetitan-crm-pp-cli <command> --help
```
...
```bash
servicetitan-crm-pp-cli <command> --agent
```
...
```bash
servicetitan-crm-pp-cli <command> --help
servicetitan-crm-pp-cli <command> --dry-run --agent
```
