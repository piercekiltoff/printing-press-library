...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "servicetitan-crm-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// ServiceTitan composed auth: every CRM endpoint requires BOTH
		// the OAuth2 bearer (set above) AND the static ST-App-Key header.
		// Without ST-App-Key the API returns 401 even with a valid bearer.
		if c.Config != nil && c.Config.AppKey != "" {
			req.Header.Set("ST-App-Key", c.Config.AppKey)
		}
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "servicetitan-crm-pp-cli/1.0")
		}
...
func resolveClientCredentials(cfg *config.Config) (string, string) {
	...
	// Defensive whitespace strip — a leading or trailing space in
	// ST_CLIENT_ID/ST_CLIENT_SECRET produces an opaque `invalid_client`
	// 400 from the ServiceTitan OAuth endpoint with no diagnostic hint.
	// Cfg.Load() already strips env-loaded values; this guards the
	// config-file path and any direct reset.
