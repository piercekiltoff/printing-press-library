...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".servicetitan-crm-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SERVICETITAN_CRM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SERVICETITAN_CRM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "servicetitan-crm-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.servicetitan-crm-pp-cli/feedback.jsonl.
When ` + "`SERVICETITAN_CRM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SERVICETITAN_CRM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servicetitan-crm-pp-cli feedback list
  servicetitan-crm-pp-cli feedback list --limit 5
  servicetitan-crm-pp-cli feedback list --json`,
