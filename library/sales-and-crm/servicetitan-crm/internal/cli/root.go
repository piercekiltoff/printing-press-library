...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "servicetitan-crm-pp-cli",
		Short: `Servicetitan Crm CLI — Every ServiceTitan CRM endpoint plus customer-360 search, lead-followup audit, and a 2-tool MCP surface that replaces t…`,
		Long: `Servicetitan Crm CLI — Every ServiceTitan CRM endpoint plus customer-360 search, lead-followup audit, and a 2-tool MCP surface that replaces t…
...
Health check: run 'servicetitan-crm-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("servicetitan-crm-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'servicetitan-crm-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBookingProviderCmd(flags))
	rootCmd.AddCommand(newBookingProviderTagsCmd(flags))
	...
	// Phase 3 transcendence top-level commands (hand-built):
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("servicetitan-crm-pp-cli %s\n", version)
		},
	}
}
