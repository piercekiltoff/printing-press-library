...
Exit codes & warnings:
  ...
  servicetitan-crm-pp-cli sync

  # Sync specific resources only
  servicetitan-crm-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  servicetitan-crm-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  servicetitan-crm-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  servicetitan-crm-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  servicetitan-crm-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/servicetitan-crm-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "booking_provider_bookings":
		return db.UpsertBookingProviderBookings(data)
	case "bookings_contacts":
		return db.UpsertBookingsContacts(data)
	case "contact_methods":
		return db.UpsertContactMethods(data)
	case "relationships":
		return db.UpsertRelationships(data)
	case "customers_contacts":
		return db.UpsertCustomersContacts(data)
	case "customers_notes":
		return db.UpsertCustomersNotes(data)
	case "customers_tags":
		return db.UpsertCustomersTags(data)
	case "dismiss":
		return db.UpsertDismiss(data)
	case "follow_up":
		return db.UpsertFollowUp(data)
	case "leads_notes":
		return db.UpsertLeadsNotes(data)
	case "locations_contacts":
		return db.UpsertLocationsContacts(data)
	case "locations_notes":
		return db.UpsertLocationsNotes(data)
	case "preferredtechnician":
		return db.UpsertPreferredtechnician(data)
	case "locations_tags":
...
func defaultSyncResources() []string {
	// Top-level CRM resources with `getList` endpoints under
	// /tenant/{tenant}/<resource>. Sub-resources (customer notes,
	// location contacts, contact methods) are reached via the per-parent
	// commands and are not synced as standalone tables; bookings is
	// included because it has its own list endpoint at /bookings.
	return []string{
		"customers",
		"locations",
		"contacts",
...
//
// ServiceTitan paths are tenant-positional: /tenant/{tenant}/<resource>. The
// tenant id is substituted from ST_TENANT_ID at call time so sync doesn't
// need a per-resource tenant flag.
func syncResourcePath(resource string) (string, error) {
	tenant := strings.TrimSpace(os.Getenv("ST_TENANT_ID"))
	if tenant == "" {
		return "", fmt.Errorf("ST_TENANT_ID is not set; required for ServiceTitan tenant-positional paths")
	}
	paths := map[string]string{
		"customers":             "/tenant/" + tenant + "/customers",
		"locations":             "/tenant/" + tenant + "/locations",
		"contacts":              "/tenant/" + tenant + "/contacts",
		"leads":                 "/tenant/" + tenant + "/leads",
		"bookings":              "/tenant/" + tenant + "/bookings",
		"booking-provider-tags": "/tenant/" + tenant + "/booking-provider-tags",
	}
