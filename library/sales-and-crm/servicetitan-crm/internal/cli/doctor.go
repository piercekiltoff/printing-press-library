...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servicetitan-crm-pp-cli doctor
  servicetitan-crm-pp-cli doctor --json
  servicetitan-crm-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export ST_CLIENT_ID=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("ST_CLIENT_ID") != "" {
				...
				authEnvInfo = append(authEnvInfo, "ST_CLIENT_ID set during auth login")
			}
			if os.Getenv("ST_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "ST_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "ST_CLIENT_SECRET set during auth login")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'servicetitan-crm-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'servicetitan-crm-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'servicetitan-crm-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'servicetitan-crm-pp-cli sync' to refresh."
	}
	return report
}
