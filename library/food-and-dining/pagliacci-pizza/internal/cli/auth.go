...
Use --chrome to read cookies from Chrome for pagliacci.com.
...
profile by name when the installed backend supports it.`,
		Example: `  pagliacci-pizza-pp-cli auth login --chrome
  pagliacci-pizza-pp-cli auth login --browser
  pagliacci-pizza-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  pagliacci-pizza-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  pagliacci-pizza-pp-cli auth login --browser\n")
				return nil
			}

			w := cmd.OutOrStdout()
			domain := "pagliacci.com"
			...
			composed := "PagliacciAuth {customerId}|{authToken}"
			for _, name := range requiredCookies {
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  pagliacci-pizza-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  pagliacci-pizza-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: pagliacci.com\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  pagliacci-pizza-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func validateComposedAuth(authHeader string) error {
	testURL := "https://pag-api.azurewebsites.net/api"
