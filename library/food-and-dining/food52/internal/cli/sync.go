...
Exit codes & warnings:
  ...
  food52-pp-cli sync --latest-only`,
	...
	// Food52-specific subcommands. The default `sync` (no args) uses the
	// generic resource-list-paginate path which Food52 doesn't expose; users
	// reach for `sync recipes` and `sync articles` for the SSR-page sync.
