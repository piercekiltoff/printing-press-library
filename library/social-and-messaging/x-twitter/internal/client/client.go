...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "x-twitter-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// X web client auth: Authorization Bearer + cookie chain + x-csrf-token
		if authHeader != "" {
			req.Header.Set("Authorization", authHeader)
		}
		if c.Config != nil {
			if cookie := c.Config.CookieHeader(); cookie != "" {
				req.Header.Set("Cookie", cookie)
			}
			if c.Config.CSRFToken != "" {
				req.Header.Set("x-csrf-token", c.Config.CSRFToken)
			}
			// Standard X web client headers; cookie auth requires these to look like the web app.
		...
		// Pretend to be a real browser: undocumented X endpoints reject default Go UA.
