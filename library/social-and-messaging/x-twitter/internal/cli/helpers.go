...
func classifyAPIError(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — usually means session cookies are missing or expired."+
			"\n      Run 'x-twitter-pp-cli auth login --paste' to refresh cookies, or export X_TWITTER_AUTH_TOKEN, X_TWITTER_CT0, X_TWITTER_GUEST_ID."+
			"\n      Run 'x-twitter-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: cookies were rejected. Refresh them via 'x-twitter-pp-cli auth login --paste'"+
			" or set X_TWITTER_AUTH_TOKEN, X_TWITTER_CT0, X_TWITTER_GUEST_ID."+
			"\n      Run 'x-twitter-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your session is valid but the request was blocked."+
			"\n      Often a templated path parameter (e.g. {pathQueryId}) was not substituted, or the endpoint needs elevated scope."+
			"\n      Run 'x-twitter-pp-cli doctor' to check auth status.", err))
