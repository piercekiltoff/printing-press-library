...
Exit codes & warnings:
  ...
  dominos-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  dominos-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  dominos-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  dominos-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  dominos-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/dominos-pp-cli/data.db)")
