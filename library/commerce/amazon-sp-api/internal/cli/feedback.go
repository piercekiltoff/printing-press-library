...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AMAZON_SP_API_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AMAZON_SP_API_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "amazon-sp-api-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.amazon-sp-api-pp-cli/feedback.jsonl.
When ` + "`AMAZON_SP_API_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AMAZON_SP_API_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amazon-sp-api-pp-cli feedback list
  amazon-sp-api-pp-cli feedback list --limit 5
  amazon-sp-api-pp-cli feedback list --json`,
