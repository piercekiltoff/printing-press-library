...
If you modify this CLI beyond what the generator produced, record each customization so it is not lost on the next regen and is visible to the next reader.
...
1. Mark every changed site in source with a comment summarizing the deviation:
    ...
    Include an upstream reference inline when there is one, for example `// PATCH(upstream cli-printing-press#<issue>): ...`. `grep -rn 'PATCH' .` from this directory then surfaces every customization.
