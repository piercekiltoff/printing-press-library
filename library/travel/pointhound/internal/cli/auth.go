...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Pointhound",
	}
...
Use --chrome to read cookies from Chrome for .pointhound.com.
...
profile by name when the installed backend supports it.`,
		Example: `  pointhound-pp-cli auth login --chrome
  pointhound-pp-cli auth login --browser
  pointhound-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  pointhound-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  pointhound-pp-cli auth login --browser\n")
				return nil
			}

			w := cmd.OutOrStdout()
			domain := ".pointhound.com"
			...
			if cookieToolSupportsProfiles(tool) {
				...
				if err != nil {
					...
					fmt.Fprintf(w, "\n  pointhound-pp-cli auth login --chrome\n")
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  pointhound-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  pointhound-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .pointhound.com\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  pointhound-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
