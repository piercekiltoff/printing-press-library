...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".flightgoat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FLIGHTGOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FLIGHTGOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "flightgoat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.flightgoat-pp-cli/feedback.jsonl.
When ` + "`FLIGHTGOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FLIGHTGOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
