...
Once synced, use the 'search' command for instant full-text search.`,
  ...
  flightgoat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  flightgoat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  flightgoat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  flightgoat-pp-cli sync --concurrency 8`,
...
func syncResourcePath(resource string) string {
	paths := map[string]string{
		"airports":          "/airports",
		"alerts":            "/alerts",
		"disruption-counts": "/disruption_counts/{entity_type}",
		"flights":           "/flights/search",
		"foresight":         "/foresight/flights/{ident}",
		"history":           "/history/flights/{ident}",
		"nearby":            "/airports/{id}/nearby",
		"operators":         "/operators",
		"routes":            "/airports/{id}/routes/{dest_id}",
		"schedules":         "/schedules/{date_start}/{date_end}",
		"weather":           "/airports/{id}/weather/observations",
	}
