...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  flightgoat-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  flightgoat-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"airports", "alerts", "disruption-counts", "flights", "foresight", "history", "nearby", "operators", "routes", "schedules", "weather"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  flightgoat-pp-cli workflow status

  # Show status as JSON
  flightgoat-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("flightgoat-pp-cli")
			}
