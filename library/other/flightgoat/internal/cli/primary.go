...
// Primary flightgoat commands: Google Flights search, cheapest-dates, and
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/flightgoat/internal/gflights"
	"github.com/mvanhorn/printing-press-library/library/other/flightgoat/internal/kayak"
...
Requires the fli CLI: pipx install flights`,
  ...
  flightgoat-pp-cli flights JFK CDG 2026-07-01 --stops non_stop --class business --json

  # Morning departures on British Airways or KLM
  flightgoat-pp-cli flights JFK LHR 2026-07-01 --time 6-12 --airlines BA,KL

  # Round trip with return date
  flightgoat-pp-cli flights SEA HNL 2026-08-01 --return 2026-08-10`,
...
Requires fli: pipx install flights`,
  ...
  flightgoat-pp-cli dates JFK CDG --from 2026-07-01 --to 2026-07-31 --stops non_stop --class business

  # Round trip with 7-day duration
  flightgoat-pp-cli dates SEA HNL --round --duration 7 --sort`,
...
duration, number of daily flights, and operating airlines.`,
  ...
  flightgoat-pp-cli explore SEA

  # Only destinations with 3+ daily flights
  flightgoat-pp-cli explore SEA --min-frequency 3

  # International only
  flightgoat-pp-cli explore SEA --country-not US --json

  # Longest nonstop flights first
  flightgoat-pp-cli explore SEA --sort duration`,
...
destinations table with durations into HTML. flightgoat parses the embedded
...
you'd see on www.kayak.com/direct/SEA in a browser.`,
  ...
  flightgoat-pp-cli longhaul SEA --min-hours 10 --country-not US --min-frequency 1

  # Medium-haul range: 5 to 8 hours
  flightgoat-pp-cli longhaul SEA --min-hours 5 --max-hours 8

  # JSON output for agents
  flightgoat-pp-cli longhaul SEA --min-hours 8 --json | jq '.routes[].code'`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			airport := strings.ToUpper(strings.TrimSpace(args[0]))
			if flags.dryRun {
