...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pagliacci-pizza-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pagliacci-pizza-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pagliacci-pizza-pp-cli workflow status

  # Show status as JSON
  pagliacci-pizza-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pagliacci-pizza-pp-cli")
			}
